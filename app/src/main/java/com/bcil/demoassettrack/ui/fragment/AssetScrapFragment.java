...
public class AssetScrapFragment extends Fragment implements  ResponseHandlerInterfaces.ResponseTagHandler, ResponseHandlerInterfaces.TriggerEventHandler, ResponseHandlerInterfaces.BatchModeEventHandler, ResponseHandlerInterfaces.ResponseStatusHandler,View.OnScrollChangeListener{
    @Bind(R.id.etScan)
    EditText etScan;
    @Bind(R.id.rvList)
    RecyclerView rvList;
    ...
    private void initView() {
        ...
        ((AppCompatActivity) getActivity()).getSupportActionBar().setTitle("Asset Scrap");
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        switch (id) {
            case android.R.id.home:
                ...
                fragmentTransaction.addToBackStack(AssetScrapFragment.class.getSimpleName());
            ...
            case R.id.btSave:
                ...
                if(assetInfoList!=null&&assetInfoList.size()>0){
                    ...
                    if (new NetworkUtils().isNetworkAvailable(Objects.requireNonNull(getActivity()))) {
                        if (checkValidation()) {
                            AsyncOnSaveScrapData asyncOnSaveScrapData = new AsyncOnSaveScrapData("OnSave", getUserName,AppConstants.EMPTY,assetInfoList1,"SCRAPPING",etReason.getText().toString().trim());
    ...
    private class AsyncValidateRfid extends AsyncTask<Object, Object, String> {
        ...
        AsyncValidateRfid(String mode, String getUserName, String scanRfid, String strData, String module , PreferenceManager preferenceManager) {
            this.mode = mode;
        ...
        protected String doInBackground(Object... objects) {
            if(!sendJsonArrayData(assetInfoList).equals("[]")){
                ...
                Objects.requireNonNull(getActivity()).runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        new CommonUtils().Alert(getActivity(),"Alert","Please select list item to scrap.");
                            ...
                            if (assetInfo != null) {
                                if (!assetInfo.getRfid().equals(rfid)) {
                                    ...
                                    Objects.requireNonNull(getActivity()).runOnUiThread(new Runnable() {
                                        @Override
                                        public void run() {
                                            AsyncValidateRfid asyncValidateRfid = new AsyncValidateRfid("ValidateRfid", getUserName, etScan.getText().toString().trim(), new AssetInfo().getJsonObject3().toString(), "SCRAPPING", preferenceManager);
